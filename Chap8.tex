...
\section{Definition of vector addition coeffieicnts}\label{sec:8.1}\index{vector-addition coefficients!definition}
...
\begin{equation}
\left[\clebsch{j_{1}}{m_{1}}{j_{2}}{m_{2}}{j}{m}\right]^{2}=\frac{2 j+1}{8 \pi} \int_{0}^{2 \pi}\dd \alpha \int_{0}^{\pi}\dd \beta \sin \beta \int_{0}^{2 \pi}\dd \gamma D_{m_{1} m_{1}}^{j_{1}}(\alpha, \beta, \gamma) D_{m_{2} m_{2}}^{j_{2}}(\alpha, \beta, \gamma) D_{m m}^{j *}(\alpha, \beta, \gamma). \label{eq:8:1:4}
\end{equation}
...
\begin{equation}
\clebsch{j_{1}}{m_{1}}{j_{2}}{m_{2}}{j}{m} \clebsch{j_{1}}{j_{1}}{j_{2}}{-j_{2}}{j}{j_{1}-j_{2}}=\frac{2 j+1}{8 \pi} \int_{0}^{2 \pi}\dd \alpha \int_{0}^{\pi}\dd \beta \sin \beta \int_{0}^{2 \pi}\dd \gamma D_{m_{1} j_{1}}^{j_{1}}(\alpha, \beta, \gamma) D_{m_{2}-j_{2}}^{j_{2}}(\alpha, \beta, \gamma) D_{m j_{1}-j_{2}}^{j *}(\alpha, \beta, \gamma), \label{eq:8:1:5}
\end{equation}
where
\[
...
\end{equation}
\section[explicit form of CG coefficients]{Explicit forms of the Clebsch-Gordan coefficients and their relations to other functions}\label{sec:8.2}\index{Clebsch-Gordan coefficient!explicit forms}
...
\end{gather}
Finite differences are analogous to differentials, while quasi-powers are analogous to ordinary powers. From this it can be concluded that the Clebsch-Gordan coefficients are analogous to the Wigner $D$-functions \cite{ref020, ref058}.\index{Wigner D-function@Wigner $D$-function}\isym{DJMM@$D_{M M^{\prime}}^{J}$, Wigner $D$-function} To illustrate this similarity let us introduce the notations
\begin{equation}
...
\end{align}
\eqref{eq:8:2:19} is similar to Eq.~\eqref{eq:4:3:4}. The analogy between \eqref{eq:8:2:17} and \eqref{eq:8:2:19} permits us to connect the recursion relations, symmetry properties, etc. of the Wigner D-functions and the Clebsch-Gordan coefficients \cite{ref058}. Since for $u \gg z, u \gg 1$ quasi-powers become asymptotically equal to ordinary powers, a Clebsch-Gordan coefficient turns into a $D$-function in the large-momentum limit, see Eq.~\eqref{eq:8:9:1}.

\subsection{Expressions for the Clebsch-Gordan Coefficients in Terms of  Binomial Coefficients \cite{ref126}}\label{sec:8.2.4}\index{Clebsch-Gordan coefficient!in terms of binomial coefficients}\index{binomial coefficients}
Ww can express the Clebsch-Gordan coeffixients in terms of a sum over binomials as
\begin{equation}
...
\subsection{Representations of the Clebsch-Gordan Coefficients in Terms of the Hypergeometric Functions}\label{sec:8.2.5}\index{Clebsch-Gordan coefficient!and hypergeometric functions}\index{hypergeometric function!generalized}
The Clebsch-Gordan coefficients may be expressed as generalized hypergeometric functions $\Fhg{3}{2}$ of unit argument,\isym{F32@$\Fhg{3}{2}$, generalized hypergeometric function}
\begin{gather}
...
& \times\Fnm[r]{3}{2}{a+\alpha+1,-a+\alpha,-c+\gamma}{-b-c+\alpha, b-c+\alpha+1}{1},
\end{aligned}, \\
& \begin{aligned}
...
& \quad \times\Fnm[r]{3}{2}{a-b-c, a-\alpha+1,-c-\gamma}{
a-b-\gamma+1,-b-c-\alpha}{1}
...
&  \label{eq:8:2:24}\\
& \quad \times\Fnm[r]{3}{2}{
a-b-c, a+b-c+1,-c-\gamma }{-2 c, a-c-\beta+1}{1} \notag\\
...
& \quad \times\Fnm[r]{3}{2}{-a-b-c-1,-a+\alpha,-c+\gamma}{-a-b+\gamma,-b-c+\alpha}{1}
. \label{eq:8:2:26}
\end{align}
Equations \eqref{eq:8:2:21}-\eqref{eq:8:2:26} may be associated with Eqs.~\eqref{eq:8:2:3}-\eqref{eq:8:2:8}, respectively.
...
\end{equation}
The quasi-binomial should be evaluated according to Eqs.~\eqref{eq:8:2:10} and \eqref{eq:8:2:11}. The variables $u$ and $v$ may be chosen in different ways \cite{ref045}:
\begin{align}
...
\times \int_{-1}^{1}(1-x)^{a-\alpha}(1+x)^{b-\beta} \frac{d^{c-\gamma}}{(d x)^{c-\gamma}}\left[(1-x)^{J-2 a}(1+x)^{J-2 b}\right]\dd x, \label{eq:8:3:1}\\
...
\times \int_{-1}^{1}(1-x)^{b+\beta}(1+x)^{b-\beta} \frac{d^{J-2 a}}{(d x)^{J-2 a}}\left[(1-x)^{c-\gamma}(1+x)^{c+\gamma}\right]\dd x. \label{eq:8:3:2}
\end{gather}
In particular, if $\alpha=\beta=\gamma=0$, then
\begin{equation}
\clebsch{a}{0}{b}{0}{c}{0}=\frac{(-1)^{a-c}}{2^{J+1} a!b!c!}\left[\frac{(J-2 b)!(J-2 c)!(J+1)!(2 c+1)}{(J-2 a)!}\right]^{\frac{1}{2}} \int_{-1}^{1}\left(1-x^{2}\right)^{b} \frac{d^{J-2 a}}{(d x)^{J-2 a}}\left[\left(1-x^{2}\right)^{c}\right]\dd x. \label{eq:8:3:3}
\end{equation}
\subsection{Integrals Involving the Wigner D-Functions}\label{sec:8.3.2}\index{integral representations!with the Wigner $D$-functions}\isym{dR@$\dd R$, invariant volume element of the rotation group}
The Clebsch-Gordan coeffieicents can be written as the integral over three Wigner $D$ functions,
\begin{equation}
\clebsch{a}{\alpha}{b}{\beta}{c}{\gamma}=\frac{1}{8 \pi^{2}}\left[\frac{(a+b+c+1)!(a+b-c)!(2 c+1)}{(2 a)!(2 b)!}\right]^{\frac{1}{2}} \int\dd R \,D_{\alpha a}^{a}(R) D_{\beta-b}^{b}(R) D_{\gamma a-b}^{c *}(R), \label{eq:8:3:4}
\end{equation}
where $R$ means the set of the Euler angles (see Sec.~\ref{sec:4.12}). We also have a more compact relation invloving one $d$ function (Akim and Levin, \cite{ref046})
\begin{equation}
\begin{aligned}
...
& \times \int_{-1}^{1}(\sin \theta)^{a+b}\left(\frac{1-\cos \theta}{1+\cos \theta}\right)^{\frac{\alpha-\beta}{2}} d_{b-a \gamma}^{c}(\theta)\, \dd \cos \theta.
\end{aligned}
\end{equation}
...
\begin{equation}
\clebsch{a}{\alpha}{b}{\beta}{c}{\gamma}=(-1)^{g-c}\left[\frac{4 \pi}{(2 a+1)(2 b+1)}\right]^{\frac{1}{2}} \frac{(g-a)!(g-b)!(g-c)!}{\Delta(a b c) g!} \int_{0}^{2 \pi}\dd \varphi \int_{0}^{\pi}\dd \vartheta \sin \vartheta Y_{a \alpha}(\vartheta, \varphi) Y_{b \beta}(\vartheta, \varphi) Y_{c \gamma}^{*}(\vartheta, \varphi). \label{eq:8:3:6}
\end{equation}
...
\begin{equation}
\clebsch{a}{\alpha}{b}{\beta}{c}{\gamma} \clebsch{a}{\alpha^{\prime}}{b}{\beta^{\prime}}{c}{\gamma^{\prime}}=\frac{2 c+1}{8 \pi^{2}} \int\dd R D_{\alpha \alpha^{\prime}}^{a}(R) D_{\beta \beta^{\prime}}^{b}(R) D_{\gamma \gamma^{\prime}}^{c *}(R). \label{eq:8:3:7}
\end{equation}
In particular,
\begin{equation}
\left[\clebsch{a}{0}{b}{0}{c}{0}\right]^{2}=\frac{2 c+1}{2} \int_{0}^{\pi}\dd \theta\, \sin \theta P_{a}(\cos \theta) P_{b}(\cos \theta) P_{c}(\cos \theta). \label{eq:8:3:8}
\end{equation}
...
\begin{align}
\threej{a}{b}{c}{\alpha}{\beta}{\gamma}&=\threej{b}{c}{a}{\beta}{\gamma}{\alpha}=\threej{c}{a}{b}{\gamma}{\alpha}{\beta} \notag\\
& =(-1)^{a+b+c}\threej{a}{c}{b}{\alpha}{\gamma}{\beta}  
 =(-1)^{a+b+c}\threej{b}{a}{c}{\beta}{\alpha}{\gamma}=(-1)^{\alpha+b+c}\threej{c}{b}{a}{\gamma}{\beta}{\alpha}. \label{eq:8:4:5}
\end{align}
...
\begin{equation}
\threej[c]{a}{b}{c}{\alpha}{\beta}{\gamma}\label{eq:8:4:7}=(-1)^{a+b+c}\threej[c]{\frac{b+c+\alpha}{2}}{\frac{a+c+\beta}{2}}{\frac{a+b+\gamma}{2}}{a-\frac{b+c-\alpha}{2}}{b-\frac{a+c-\beta}{2}}{c-\frac{a+b-\gamma}{2}}
\end{equation}
(corresponding to the permutation of the first and third rows of the $R$-symbol) and\\
\item
\begin{equation}
\threej{a}{b}{c}{\alpha}{\beta}{\gamma}\label{eq:8:4:8}=\threej[c]{a}{\frac{b+c-\alpha}{2}}{\frac{b+c+\alpha}{2}}{-b+c}{\frac{b-c-\alpha}{2}-\gamma}{\frac{b-c+\alpha}{2}+\gamma}
\end{equation}
...
\begin{align}
& \threej[c]{a}{b}{c}{\alpha}{\beta}{\gamma}=\threej[c]{a}{\frac{b+c-\alpha}{2}}{\frac{b+c+\alpha}{2}}{c-b}{\frac{b+\beta-c-\gamma}{2}}{\frac{b-\beta-c+\gamma}{2}}=\threej[c]{b}{\frac{a+c-\beta}{2}}{\frac{a+c+\beta}{2}}{a-c}{\frac{-a-\alpha+c+\gamma}{2}}{\frac{-a+\alpha+c-\gamma}{2}}  \notag\\
& =\threej[c]{c}{\frac{a+b-\gamma}{2}}{\frac{a+b+\gamma}{2}}{b-a}{\frac{a+\alpha-b-\beta}{2}}{\frac{a-\alpha-b+\beta}{2}}=\threej[c]{\frac{b+c-\alpha}{2}}{\frac{a+c-\beta}{2}}{\frac{a+b-\gamma}{2}}{a-\alpha-\frac{b+c-\alpha}{2}}{b-\beta-\frac{a+c-\beta}{2}}{c-\gamma-\frac{a+b-\gamma}{2}}  \notag\\
& =\threej[c]{\frac{b+c+\alpha}{2}}{\frac{a+c+\beta}{2}}{\frac{a+b+\gamma}{2}}%
...
\end{align}
\section[Special Values of the arguments]{Explicit forms of the Clebsch-Gordan coefficients for special values of the arguments}\label{sec:8.5}\index{Clebsch-Gordan coefficient!for special values of the arguments}
...
\end{align}
Here $a-b \geq|\alpha-\beta| \geq 0, b^{\prime}$ is integer, $b^{\prime}+2 b$ is even, and $a-b+c \geq b^{\prime} \geq|-a+b+c|, 2 b \geq b^{\prime} \geq|2 \beta|$.
...
Recursion relations for the Regge $R$-symbols are very convenient because each relation is equivalent to a variety of formally different expressions for the Clebsch-Gordan coefficients. The formulas given below represent some recursion relations \cite{ref086,ref103}. In these formulas we use the notation $J=\sum_{k=1}^{3} R_{i k}=\sum_{i=1}^{3} R_{i k}=a+b+c$.
\begin{align}
...
R_{31} & R_{32} & R_{33}
\end{array}\right\|=0 .
\footnote{Column recursion — it shifts $R_{1k}+1$ and $R_{2k}-1$, so each array has equal column sums but row sums J+1, J−1, J. A Regge symbol requires all six line-sums equal, so every term vanishes as a standard 3jm and the relation is only vacuously satisfied.  It's not an OCR issue; the transcription matches the scan.}
\label{eq:8:6:36}\\
...
\end{align}
\section{Sums of products of Clebsch-Gordan coefficients}\label{sec:8.7}\index{Clebsch-Gordan coefficient!sums of products of}\index{sums!of the Clebsch-Gordan coefficients}
...
\sum_{\alpha \beta \delta} \clebsch{b}{\beta}{c}{\gamma}{a}{\alpha} \clebsch{b}{\beta}{e}{\varepsilon}{d}{\delta} \clebsch{a}{\alpha}{f}{\varphi}{d}{\delta}=\kappa_{1} \frac{\Fact{a\dd d}}{\Fact{e}} \clebsch{c}{\gamma}{f}{\varphi}{e}{\varepsilon}\sixj{a}{b}{c}{e}{f}{d}, \label{eq:8:7:13}\\
...
\sum_{\alpha \beta \delta}(-1)^{b+\beta} \clebsch{a}{\alpha}{b}{\beta}{c}{\gamma} \clebsch{b}{-\beta}{e}{\varepsilon}{d}{\delta} \clebsch{a}{\alpha}{f}{\varphi}{d}{\delta}=\kappa_{1} \frac{\Fact{c\dd d}}{\Fact{e}} \clebsch{c}{\gamma}{f}{\varphi}{e}{\varepsilon}\sixj{a}{b}{c}{e}{f}{d}, \label{eq:8:7:16}\\
...
& =(-1)^{a-b+c+d+e-f} \sum_{s \sigma} \Fact{s s a g} \clebsch{a}{\alpha}{s}{\sigma}{j}{\mu} \clebsch{g}{\eta}{s}{\sigma}{d}{\delta}\sixj{b}{c}{a}{j}{s}{f}\sixj{b}{e}{g}{d}{s}{f}=\Fact{a\dd g j} \sum_{k \kappa} \clebsch{g}{\eta}{j}{\mu}{k}{\kappa} \clebsch{d}{\delta}{a}{\alpha}{k}{\kappa}\ninej{c}{b}{a}{f}{e}{d}{j}{g}{k}, \label{eq:8:7:20}\\
& \sum_{\beta \gamma \varepsilon \varphi} \clebsch{b}{\beta}{c}{-\gamma}{a}{\alpha} \clebsch{e}{\varepsilon}{f}{-\varphi}{d}{\delta} \clebsch{b}{\beta}{e}{\varepsilon}{g}{\eta} \clebsch{c}{\gamma}{f}{\varphi}{j}{\mu}=(-1)^{b+e-g} \Fact{a\dd g j} \sum_{k \kappa} \clebsch{g}{\eta}{j}{-\mu}{k}{\kappa} \clebsch{d}{\delta}{a}{\alpha}{k}{\kappa}\ninej{c}{b}{a}{f}{e}{d}{j}{g}{k}, \label{eq:8:7:21}\\
...
& \sum_{\beta \gamma \varepsilon \varphi} \clebsch{b}{\beta}{c}{\gamma}{a}{\alpha} \clebsch{e}{\varepsilon}{f}{\varphi}{d}{\delta} \clebsch{e}{\varepsilon}{g}{\eta}{b}{\beta} \clebsch{f}{\varphi}{j}{\mu}{c}{\gamma}=\sum_{k \kappa} \Fact{b c\dd k} \clebsch{g}{\eta}{j}{\mu}{k}{\kappa} \clebsch{d}{ \delta}{k}{\kappa}{a}{\alpha}\ninej{a}{b}{c}{d}{e}{f}{k}{g}{j}, \label{eq:8:7:26}\\
& \sum_{\beta \gamma \varepsilon \varphi} \clebsch{b}{\beta}{c}{-\gamma}{a}{\alpha} \clebsch{e}{\varepsilon}{f}{-\varphi}{d}{\delta} \clebsch{g}{\eta}{b}{-\beta}{e}{\varepsilon} \clebsch{j}{\mu}{f}{-\varphi}{c}{\gamma}=(-1)^{c+e-g+j+\alpha-\mu} \Fact{a\dd e c} \sum_{k \kappa} \clebsch{g}{\eta}{j}{-\mu}{k}{\kappa} \clebsch{d}{\delta}{a}{\alpha}{k}{\kappa}\ninej{c}{b}{a}{f}{e}{d}{j}{g}{k}, \label{eq:8:7:27}\\
...
& \sum_{c \gamma}(-1)^{e+d-\beta} \frac{\Fact{c\dd d}}{\Fact{e}} \clebsch{a}{\alpha}{b}{\beta}{c}{\gamma} \clebsch{f}{\varphi}{c}{\gamma}{e}{\varepsilon}\sixj{a}{b}{c}{e}{f}{d}=\clebsch{b}{-\beta}{e}{\varepsilon}{d}{\delta} \clebsch{a}{\alpha}{f}{\varphi}{d}{\delta}, \label{eq:8:7:37}\\
...
& \sum_{g j}(-1)^{g-b-f+\varphi+\varepsilon} \frac{\Fact{a\dd j j g g}}{\Fact{b c}} \clebsch{g}{\eta}{j}{-\mu}{k}{\kappa} \clebsch{e}{-\varepsilon}{g}{\eta}{b}{\beta} \clebsch{f}{\varphi}{j}{\mu}{c}{\gamma}\ninej{c}{b}{a}{f}{e}{d}{j}{g}{k}=\clebsch{b}{\beta}{c}{-\gamma}{a}{\alpha} \clebsch{e}{\varepsilon}{f}{\varphi}{d}{\delta} \clebsch{d}{\delta}{a}{\alpha}{k}{\kappa} . \label{eq:8:7:43}
\end{align}
...
\end{equation}
In these equations $l, l^{\prime}, J$ are integers.
...
\section{Generating functions of the Clebsch-Gordan coefficients}\label{sec:8.8}\index{Clebsch-Gordan coefficient!generating functions}\index{generating functions!for the Clebsch-Gordan coefficients}
...
\section[Classical limit of CG coeeficients]{Classical limit and asymptotic expressions for the Clebsch-Gordan coefficients}\label{sec:8.9}\index{Clebsch-Gordan coefficient!classical limit}\index{Clebsch-Gordan coefficient!asymptotic expressions}\index{asymptotics!of the Clebsch-Gordan coefficients}\index{classical limit}
...
\]
\section[Relation to other authors]{Connection of the Clebsch-Gordan coefficients and the $3 jm$ symbols with analogous functions of other authors}\label{sec:8.11}\index{Clebsch-Gordan coefficient!notations of other authors}\index{notation!of other authors}
\[
...
Below we present on a small selection of tables of algebraic formulas for the Clebsch-Gordan coefficients $\clebsch{a}{\alpha}{b}{\beta}{c}{\gamma}$ with $b=1 / 2,1$. The original book contains many more. However, the use of Mathematica or SymPY will allow others to be evaluated quickly.
Algebraic tables of the Clebsch-Gordan coefficients are also available in Ref.~\cite{ref010} for $b=1 / 2,1,3 / 2,2$; in Refs.~\cite{ref121,ref125} for $b=5 / 2$; in Refs.~\cite{ref117,ref130} with $b=3$; and in Ref.~\cite{ref126} with $b=7 / 2,4,9 / 2,5$.
...
Even though the original book provides numerical values of the Clebsch-Gordan coefficients, with the advent of exact evaluators (as in sympy), these are no longer very useful.
