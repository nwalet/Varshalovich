...
\newcommand{\dS}[4] {
     {acos(cos(#1)*cos(#3)+sin(#1)*sin(#3)*cos(#2-#4))}
}
...
\begin{tikzpicture}[scale=1.15]
  ...
  \coordinate (P1) (\ToXYZ{\phia}{\thetaa});
  \coordinate (P2) (\ToXYZ{\phib}{\thetab});
  \coordinate (P3) (\ToXYZ{\phib}{\thetac});
  \draw[bvec] (O) --($1.2*(P1)$);
  \draw[bvec] (O) --($1.2*(P2)$);
  \draw[bvec] (O) --($1.2*(P3)$);

  \tikzmath{
    \dsa = \dS(\phib,\thetab,\phic,\thetac);
    \dsb = \dS(\phic,\thetac,\phia,\thetaa);
    \dsc = \dS(\phia,\thetaa,\phib,\thetab);
    \ssca = 1/sin(\dsa);
    \sscb = 1/sin(\dsb);
    \sscc = 1/sin(\dsc);
   };
