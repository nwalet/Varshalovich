...
The normalization of state vectors may be different. In diagrams it is indicated by the type of node. A state vector \(\ket{j m}\) normalized according to
\[
...
A Wigner \(3 j m\) symbol \(\threej{a}{b}{c}{\alpha}{\beta}{\gamma} \equiv\rbraket{00}{a \alpha b \beta c \gamma}\) is represented by a dot-node junction of three thin solid lines which correspond to \(\ket{a \alpha}, \ket{b \beta}\) and \(\ket{c \gamma}\). The sign of the node determines the cyclic order of arguments, i.e., the sequence of coupling of momenta: plus corresponds to the anticlockwise one.
...
Matrix elements of any irreducible tensor operator \(\braOket{\Psi}{T_{\lambda \mu}}{\Psi^{\prime}}\) are represented by a block with three external lines corresponding to \(\bra{\Psi}\), \(ket{\Psi^{\prime}}\) and \(\ket{\lambda \mu}\); the operator symbol (and other possible quantum numbers) are indicated inside the block.
...
``ket''  \( \begin{aligned} & \ket{j m} \\ & \ket{j m} \end{aligned} \) & \includegraphics[valign=c]{fig11_1_9}
\\
...
``ket''  \( \begin{aligned} & \ket{\Omega} \\ & \ket{\Omega} \end{aligned} \) & \includegraphics[valign=c]{fig11_1_13}
\\
...
\begin{center}
\captionsetup{justification=centering,labelformat=empty}
\caption{Table 11.11.\\ Metric Tensor for \((2j+1)\)-Dimensional Space of Functions \(\ket{jm}\).}
\small
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.3}
...
\(\ket{jm^{\prime}\,jm}=(-1)^{2j}\ket{jm\,jm^{\prime}}=\bra{ jm^{\prime}\,jm}=(-1)^{2j}\bra{ jm\,jm^{\prime}}\)
& \begin{tabular}{@{}c@{}}
...
\] &
\scalebox{0.5}{
  \redlhs{\mathfrak{N}^{(2)}_{\gamma\gamma'}}{j_1}{j_2}{j'_1}{j'_2}
\(=\)
...
(d) A diagram with three external \(j\)-lines which represent \(\ket{j_{1} m_{1}}, \ket{j_{2} m_{2}}\) and \(\ket{j_{3} m_{3}}\) is proportional to the \(3 j m\) symbol \(\threej{j_{1}}{j_{2}}{j_{3}}{m_{1}}{m_{2}}{m_{3}}\)
\[
...
where
\[
\braOket{j_{1} j_{2} j m}{\mathfrak{M}}{j_{3} j_{4} j m} \equiv \sum_{m_{1} m_{2} m_{3} m_{6}}\threej{j_{1}}{j_{2}}{j}{m_{1}}{m_{2}}{m}\left(j_{1} m_{1} j_{2} m_{2}|\mathfrak{M}| j_{3} m_{3} j_{4} m_{4}\right)\threej{j_{3}}{j_{4}}{j^{\prime}}{m_{3}}{m_{4}}{m^{\prime}}
\]
\begin{center}
