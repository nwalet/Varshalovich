...
% styles for the Sec. 9.9 tetrahedron figures (Figs. 9.1-9.3)
\tikzset{
...
\item All momenta are integer or half-integer nonnegative numbers (with one exception considered in Sec. 9.4).
...
\end{equation}
In Eq. \eqref{eq:9:2:13} the sum is over all possible values of $\alpha, \beta, \gamma, \delta, \varepsilon, \varphi$ with only three summation indices being independent. Some other sums of the $3 j m$ symbols which yield the $6 j$ symbols will be considered in Chap. 12 .
...
\end{equation}
where $d_{m n}^{f}(\Theta)$ is the rotation matrix (Chap. 4), $\Theta$ is an angle between the tetrahedron edges $a+n+\frac{1}{2}$ and $b+m+\frac{1}{2}$ (Fig.~\ref{chap9:fig:2})
\begin{equation}
