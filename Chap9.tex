...
\end{equation}
Similarly, the state vectors corresponding to the coupling scheme III are eigenvectors of the operators $\widehat{\vect{j}}_{\frac{1}{2}}^{2}, \widehat{\vect{j}}_{2}^{2}, \widehat{\vect{j}}_{3}^{2}$, $\widehat{j}_{13}^{2}, \widehat{j}^{2}, \widehat{j}_{x}$,
\begin{equation}
...
\begin{equation}
\sixj{a}{b}{c}{d}{e}{f}  \label{chap9:eq:32}=\sum(-1)^{d+e+f+\delta+e+\varphi}\left(\begin{array}{lll}
a & b & c \\
\alpha & \beta & \gamma
...
\end{array}\right) .
\end{equation}
In Eq. \eqref{chap9:eq:32} the sum is over all possible values of $\alpha, \beta, \gamma, \delta, \varepsilon, \varphi$ with only three summation indices being independent. Some other sums of the $3 j \mathrm{~m}$ symbols which yield the $6 j$ symbols will be considered in Chap. 12 .
...
\begin{equation}
\sixj{a}{b}{c}{d+R}{e+R}{f+r}  \label{chap9:eq:105} \approx \frac{(-1)^{a+b+c+2(d+e+f)}}{\sqrt{2 R}}\left(\begin{array}{ccc}
a & b & c \\
e-f & f-d & d-e
\end{array}\right) .
\end{equation}
...
\subsubsection{(a) The Ponzano-Regge Formula [89] (semiclassical approximation to the $6 j$ symbols)}
...
\subsubsection{(b) The Edmonds' Formula [16]}
...
\end{equation}
\subsubsection{(c) If $a, b, c, d, e, f \gg 1, m, n, p$ one has}
\begin{equation}
...
\subsubsection{(d) In particular, for $m=n=p=0$ Eq. \protect\eqref{chap9:eq:123} yields the Wigner formula [43]}
\begin{equation}
...
\subsubsection{(e) If $a, b, c$, etc. are fixed and $R \rightarrow \infty$, one has [89]}
\begin{align}
